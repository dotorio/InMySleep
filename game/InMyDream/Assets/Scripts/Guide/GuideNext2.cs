...
public class GuideNext2 : MonoBehaviourPunCallbacks
{
    ...
    public GameObject Allow11; // 화살표 오브젝트 2-1
    public GameObject Allow12;
    public GameObject Allow21;
    public GameObject Allow31;
    public GameObject Allow41;
    ...
    public void NextImageSequence()
    {
        if (currentIndex < images.Length - 1 && currentIndex < texts.Length - 1)
        {
            ...
            if (currentIndex == 0)
            {
                Allow11.SetActive(true); // 화살표1 활성화
                Allow12.SetActive(true); // 화살표1 활성화
                StartPoint.SetActive(true); // 화살표1 활성화
                EndPoint.SetActive(true); // 화살표1 활성화
            }
            else
            {
                Allow11.SetActive(false); 
                Allow12.SetActive(false); 
            ...
            if (currentIndex == 2)
            {
                Allow31.SetActive(true); // 화살표2 활성화
            }
            else
            {
                Allow31.SetActive(false); // 화살표2 비활성화
            }

            if (currentIndex == 3)
            {
                Allow41.SetActive(true); // 화살표2 활성화
            }
            else
            {
                Allow41.SetActive(false); // 화살표2 비활성화
            }
        }
    }
