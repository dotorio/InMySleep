...
public class CatController : MonoBehaviour
{
    ...
    void BallThrow()
    {
        ...
        // 발사체 생성
        GameObject projectile = Instantiate(ball, hand.position, hand.rotation);
        ...
        if (cnt%2 == 0)
        {
            Vector3 launchDirection = CalculateLaunchDirection(hand.position, player2.transform.position, 45f);
            rb.velocity = launchDirection;
        }
        else
        {
            Vector3 launchDirection = CalculateLaunchDirection(hand.position, player1.transform.position, 45f);
            rb.velocity = launchDirection;
        }
