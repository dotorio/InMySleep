...
public class ThirdPersonController : MonoBehaviourPun
{
    ...
    // 미끄러짐 시작
    void StartSliding(Collider slopeCollider)
    {
        isSliding = true;

        // 경사면의 방향을 계산
        RaycastHit hit;
        if (Physics.Raycast(slopeCollider.transform.position, Vector3.down, out hit, 2f))
        {
    ...
    void StopSliding()
    {
        StartCoroutine(DelayedStopSliding());
    }
    ...
    void HandleMovement()
    {
        ...
        if ((horizontal != 0f || vertical != 0f) && isGrounded && canPickUp && !isSliding)
        {
        ...
        if (direction.magnitude >= 0.1f && !isSliding)
        {
